...
class ClockEngine:
    ...
    def run(self, duration_sec):
        ...
        while time.time() - start_time < duration_sec:
            ...
            elif self.config.time_theme == 21:
            ...
            if self.config.time_theme == 22:
                ...
                # Binary Clock
                img = self.binary_clock.tick(img, time_str, font, self.config.clock_color_1, self.config.clock_color_2)
            elif self.config.time_theme == 26:
                # Pac-Man Clock
                img = self.pacman_clock.tick(img, time_str, font, self.config.clock_color_1, self.config.clock_color_2)
                ...
                if not hasattr(self, 'prev_digits'):
                    self.prev_digits = [""] * 8  # HH:MM:SS is 8 chars
                ...
                if is_flipping:
                    ...
                    for flip_frame in range(1, 9):
                        ...
                        start_x = (self.config.matrix_width - total_w) // 2 + self.config.time_offset_x
                        y_pos = (self.config.matrix_height - panel_h) // 2 + self.config.time_offset_y
                        ...
                        for i, char in enumerate(time_chars):
                            ...
                            if is_flipping_panel:
                                ...
                                # Scale shrink to panel height
                                shrink_px = int((shrink / 4.0) * (panel_h / 2))
                                
                                # Draw shrinking white panel
                                ...
                                # Draw text inside
                                draw_styled_text(anim_img, char, (cx + 1, y_pos + 1), font, 19, self.config.clock_color_1, self.config.clock_color_2, scale=scale_factor)
                                # Middle line
                                mid_y = y_pos + panel_h // 2
                                anim_draw.line([(cx, mid_y), (cx + panel_w - 1, mid_y)], fill=(0, 0, 0), width=1)
                ...
                for i, char in enumerate(time_chars):
                    ...
                    draw_styled_text(img, char, (cx + 1, y_pos + 1), font, 19, self.config.clock_color_1, self.config.clock_color_2, scale=scale_factor)
                    mid_y = y_pos + panel_h // 2
                    draw.line([(cx, mid_y), (cx + panel_w - 1, mid_y)], fill=(0, 0, 0), width=1)
                    cx += panel_w + spacing
            else:
                # Normal themes text drawing
                draw_styled_text(img, time_str, (x, y), font, self.config.time_theme, self.config.clock_color_1, self.config.clock_color_2, scale=scale_factor)
